...
const Factory = () => {
  ...
  const handleViewClick = (item) => {
    navigate('/factories/view', { state: item });
  };
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/factories/?is_trashed=${istrashed}`, {
        method: 'GET', 
        headers: {
          'Authorization': `Bearer ${token}`
        }
                ...
                {data.map((product) => (
                  ...
                  <td>
                    {/* Action buttons or icons */}
                      ...
                      <button type="button" className="btn mybtncustomer btn-secondary btn-sm mr-2" onClick={() => handleViewClick(product)}><i className="bi bi-eye-fill my-eye-color" /></button>
                      <button type="button" className="btn mybtncustomer btn-secondary btn-sm mr-2"  onClick={() => handleEditAddress(product.id)}> <i className="bi bi-geo-alt-fill my-geo-color" /> </button>
