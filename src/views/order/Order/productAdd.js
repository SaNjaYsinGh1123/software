...
const Add = () => {
  ...
  const apiCall = async () => {
    try {
      ...
      if (response.status === 201) {
        ...
        setsubmitBlock(true);
        frontsubmitsuccess = datas.product_id
  ...
  const handleSubmit = async () => {
    ...
    if (validateForm()) {
      ...
      const result = await apiCall();  // Wait for the apiCall to resolve
  ...
  const handleSubmitFront = async (event) => {
    event.preventDefault();
    if (validateForm()) {
      console.log('Form is valid, proceed with API call');
      const result = await apiCall();  // Wait for the apiCall to resolve
      ...
      <Row>
        ...
        <Col md="12">
          <Card>
            ...
            <CardBody>
              <Form onSubmit={handleSubmitFront}>
                <Row>
                  <Col md="10">
                    <FormGroup>
                      <Label>Grain</Label>
                  ...
                  <Col md="10">
                    <FormGroup>
                      ...
                      <AsyncSelect
                        ...
                        isSearchable
                      />
                      {errors.fabricId && (formDatas.fabricId === null || formDatas.fabricId.value === 'x' ) && (
                      ...
                      <table className="table">
                        ...
                        <tbody>
                          {items1.map((item, index) => (
                            <tr key={item.design_id + item.shade_id}>
                              <Row>
                                <Col md="4">
                                  <AsyncSelect
                                    name="design_id"
              ...
              {formDatas.isOnlineProduct === '1' && (
                <ProductBackSideAdd
                  ...
                  fronthandleSubmit={handleSubmit}
                />
              )}
            </CardBody>
          </Card>
