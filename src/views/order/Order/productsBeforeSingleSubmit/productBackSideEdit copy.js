...
const ProductBackSideEdit = ({frontSidedata,backSideProduct,FrontSubmitBlock,data7,data8}) => {
  const navigate = useNavigate();

  console.log(frontSidedata,backSideProduct,data7,data8);
  ...
  const [formDatas, setFormDataS] = useState({
    ...
    hsnId: { value: hsnId, label: hsnName },
    quantity,
    PricePerUnit: price,
    Thickness,
    TaxRate,
  ...
  const handleSubmit = async (event) => {
    ...
    if (validateForm()) {
      ...
      console.log('Form is invalid, do not submit test');
    }
  };
