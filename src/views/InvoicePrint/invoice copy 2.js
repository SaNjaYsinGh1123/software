    ...
    doc.autoTable({
        ...
        body: [
            ...
            [
                '1',
                {
                    content: 'Textile Fabric Coated with PVC (Rexine) \n CRN: 56  Front Side 1001 A | E102 | ACR | 23 mm | apple | P013 / P013 / Neutral \n Additional Treatments 12356 \n Back Side \n 1002 A | ACR | GRY E123',
                    colSpan: 1, // Merge across 7 columns
                },
                'Ist','39209999', '8.28', '70 m', '45%', '260.82', '579.60'
            ],

            ...data?.autoInvoiceitems.map((item, index) => [
                index + 1,
                {
                    content: ` ${item?.productDetails?.hsn_name}  ${item?.productDetails?.customer_item_reference !== '' ? `CRN: ${item?.productDetails?.customer_item_reference}`:""}  Front Side ${item?.productDetails?.grain_name} | ${item?.productDetails?.emboss_name} | ${item?.productDetails?.fabric_name} | ${item?.productDetails?.thickness}mm | ${item?.productDetails?.color_name} \n Back Side \n 1002 A | ACR | GRY E123 \n`,
                    colSpan: 1,
                },
                `${item.grade} (${item.discount_rate}%) Disc`,
                item.hsn_code,
                (Number(item.price) -((Number(item.price) * Number(item.discount_rate))/100)).toFixed(2),
                item.quantity,
                item.tax_rate,
                (((Number(item.price) -((Number(item.price) * Number(item.discount_rate))/100)).toFixed(2)) * Number(item.quantity) * Number(item.tax_rate))/100,
                ((Number(item.price) -((Number(item.price) * Number(item.discount_rate))/100)).toFixed(2) * Number(item.quantity))
            ]),
...
function numberToWords(numAmount) {
    ...
    const generatePDF = (temp,OrderResult,addr,addr1) => {
        ...
        temp.autoInvoiceitems.forEach((item)=>{
                const  tempPrice = (Number(item.price) -((Number(item.price) * Number(item.discount_rate))/100)).toFixed(2)
                calc.totalQty += Number(item.quantity);
                calc.totalTax += (tempPrice * Number(item.quantity) * Number(item.tax_rate))/100;
                calc.subTotal += (tempPrice * Number(item.quantity));
