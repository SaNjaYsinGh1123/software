...
const PasteTypes = () => {
  ...
  const handleEditClick = (item) => {
    ...
    navigate('/resources/paste-types/edit', { state: item });
  };
  const handleEditAdd = () => {
    
    navigate('/resources/paste-types/add');
  };
  const handleTrash = ()=>{
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/pasteteams/?is_trashed=${istrashed}`, {
        method: 'GET', 
        headers: {
          'Authorization': `Bearer ${token}`
        }
