...
const HsnCodes = () => {
  ...
  const handleEditClick = (item) => {
    ...
    navigate('/resources/hsn-codes/edit', { state: item });
  };
  const handleEditAdd = () => {
    
    navigate('/resources/hsn-codes/add');
  };
  const handleTrash = ()=>{
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/hsns/?is_trashed=${istrashed}`, {
        method: 'GET', 
        headers: {
          'Authorization': `Bearer ${token}`
        }
