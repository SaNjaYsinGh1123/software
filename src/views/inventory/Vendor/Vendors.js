...
const Customer = () => {
  ...
  const handleEditClick = (item) => {
    ...
    navigate(`/inventory/vendors/edit/?id=${item.id}`);
  };
  ...
  const handleView = (customer) => {
    navigate(`/inventory/vendors/view/?id=${customer.id}`);
  };

  const handleEditAddress = (customer) => {
    navigate(`/inventory/vendors/address/?id=${customer.id}`);
  };

  const handlePendingReport = (customer) => {
    navigate(`/inventory/vendors/pending-report/?id=${customer.id}`);
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/vendors/?page=${currentPage}&limit=10&title=company_name&order=asc`, {
        method: 'GET',
        headers: {
      ...
      const sortedCustomers  = result?.vendor?.sort((a, b) => {
