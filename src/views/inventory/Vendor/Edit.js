...
const handleInputChange = (index, event) => {
  ...
  const handleEditAddress = ()=>{
   console.log('address');
    navigate(`/inventory/vendors/address/?id=${id}`);
  }
  ...
  async function apiCall() {
    try {
        ...
        const bodyData ={
          ...
          vendor_company_representative: filtered,
          vendor_company_document:compdoctemp,
          is_trashed:'0'
        }
        const token = localStorage.getItem('userToken');
        ...
        if(response.status === 200){
          ...
          if(datas){
            setFormDataS(prevState => ({
              ...
              items:datas.vendorcompanyrepresentatives,
              compdoc: datas?.vendorcompanydocuments?.length === 0 ? datas.vendorcompanydocuments:[{name:'',file_path:'',image_id:''}]
            }));
