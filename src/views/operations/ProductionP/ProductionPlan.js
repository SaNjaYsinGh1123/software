...
const ProductionPlan = () => {
  ...
  useEffect(() => {
    const fetchDataPlan = async () => {
      ...
      const response = await fetch(`https://factory.teamasia.in/api/public/productionplan/getPlanDates/?page=${currentPage}&limit=10&title=date&order=desc`, {
        method: 'GET',
        headers: {
          ...
          <tbody>
            {filteredData.map((plan) => (
              <tr key={plan.date}>
                ...
                <td>{plan.created_by !== ''?plan.created_by:'testing team'}</td>
                <td>Published</td>
