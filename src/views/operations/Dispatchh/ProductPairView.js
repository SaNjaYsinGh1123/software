...
const JumbotronComponent = (props) => {
  ...
  const [checkboxState, setCheckboxState] = useState(() => {
    const cookieData = Cookies?.get('orderCheckboxes');
    return cookieData ? JSON.parse(cookieData) : {};
  });


  const checkboxclick = (orderId,frontProductId,checked) => {
    // console.log('hi');
    setCheckboxState(prevState=>{
      ...
      const updatedCheck = {
          ...
          [orderId]:{
            ...prevState[orderId],
            [frontProductId]:checked?1:0
          }
        }
        Cookies?.set('orderCheckboxes', JSON.stringify(updatedCheck), { expires: 7 });
        return updatedCheck;
      })
    // setCheckboxState({8 : { 10:1}});
    
   
  };
  
  // console.log(Cookies?.get('orderCheckboxes'));
  ...
  return (
    <>
        <div className="boxShadow">
          <div className='table-margin-temp' >
            <Table className='table-margin-zero order-table-button' size="sm" style={{ background: '#e3e3e3', padding: '2px',margin:'10px',width: '98%' }}>
              
            <Row style={{ background: 'rgb(255 255 255)',borderRadius:'10px 10px 0px 0px',borderTop:'1px solid #2d2c2c2b',borderLeft:'1px solid #2d2c2c2b',borderRight:'1px solid #2d2c2c2b'}}>
                <Col md="2">
                    <div className='fix-wid-1-temp'>
                      ...
                      <Input 
                        type="checkbox"
                        checked={checkboxState[data?.front_side?.order_id] && checkboxState[data?.front_side?.order_id][data?.front_side?.id] === 1}
                        onChange={(e)=>checkboxclick(data?.front_side?.order_id,data?.front_side?.id,e.target.checked)}
                      />
            ...
            <Table className='table-margin-zero' responsive size="sm">
              <thead>
                <tr>
                  <th colSpan={13} style={{ background: '#777', textAlign: 'center', color: 'white',padding: '0px',borderRadius:'10px'}}>{data.front_side.is_online_product === '1' && data.front_side.ref_product_id === '0' && 'Front Side'}{data.front_side.is_online_product === '0' && data.front_side.ref_product_id === '0' && ''}</th>
                </tr>
                <tr> 
              ...
              <tbody>
                <tr>
                  ...
                  <td>{printing(data?.front_side?.productprints)?.map(item=> <div key={item}>{item}</div>)}</td>
          ...
          {Object.keys(data.back_side).length  > 0 && (
            <div>
              <Table className='table-margin-zero' responsive size="sm">
                <thead>
                  <tr>
                    <th colSpan={13} style={{ background: '#777', textAlign: 'center', color: 'white',padding: '0px',borderRadius:'10px'}}>Back Side</th>
                  </tr>
                  <tr>
                    <th scope="col">Grain</th>
                    <th scope="col">Color</th>
                ...
                <tbody>
                    <tr >
                      ...
                      <td>{printing(data?.back_side?.productprints)?.map(item=> <div key={item}>{item}</div>)}</td>
                      <td>N/A</td>
