...
// console.log('printArray',printArray);
  ...
  useEffect(() => {
    ...
    const fetchData = async (ProductData) => {
      console.log('backProductData',ProductData);
      let url = 0;
      if(ProductData.is_online_product === '1' && ProductData.ref_product_id === '0'){
         url = ProductData.id
      }else{
         url = ProductData.ref_product_id
      }
      const token = localStorage.getItem('userToken');
      const response = await fetch(`https://factory.teamasia.in/api/public/products/pair/${url}`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`
        }
